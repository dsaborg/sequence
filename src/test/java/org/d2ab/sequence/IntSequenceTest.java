...
public class IntSequenceTest {
	...
	public void skip() {
		...
		expecting(NoSuchElementException.class, () -> threeSkipThree.iterator().nextInt());
		expecting(NoSuchElementException.class, () -> threeSkipFour.iterator().nextInt());

		assertThat(removeFirst(threeSkipOne), is(2));
		twice(() -> assertThat(threeSkipOne, containsInts(3)));
		twice(() -> assertThat(_123, containsInts(1, 3)));
	}
	...
	public void skipTail() {
		...
		expecting(NoSuchElementException.class, () -> threeSkipTailThree.iterator().nextInt());
		...
		IntSequence nineSkipTailNone = _123456789.skipTail(0);
		twice(() -> assertThat(nineSkipTailNone, is(sameInstance(_123456789))));

		IntSequence nineSkipTailOne = _123456789.skipTail(1);
		twice(() -> assertThat(nineSkipTailOne, containsInts(1, 2, 3, 4, 5, 6, 7, 8)));

		IntSequence nineSkipTailTwo = _123456789.skipTail(2);
		twice(() -> assertThat(nineSkipTailTwo, containsInts(1, 2, 3, 4, 5, 6, 7)));

		IntSequence nineSkipTailThree = _123456789.skipTail(3);
		twice(() -> assertThat(nineSkipTailThree, containsInts(1, 2, 3, 4, 5, 6)));

		IntSequence nineSkipTailFour = _123456789.skipTail(4);
		twice(() -> assertThat(nineSkipTailFour, containsInts(1, 2, 3, 4, 5)));
	}
	...
	public void limit() {
		...
		IntIterator iterator = threeLimitedToOne.iterator();
		iterator.nextInt();
		expecting(NoSuchElementException.class, iterator::nextInt);
	...
	public void limitTail() {
		IntSequence threeLimitTailToNone = _123.limitTail(0);
		twice(() -> assertThat(threeLimitTailToNone, is(emptyIterable())));
		expecting(NoSuchElementException.class, () -> threeLimitTailToNone.iterator().nextInt());

		IntSequence threeLimitTailToOne = _123.limitTail(1);
		twice(() -> assertThat(threeLimitTailToOne, containsInts(3)));
		IntIterator iterator = threeLimitTailToOne.iterator();
		iterator.nextInt();
		expecting(NoSuchElementException.class, iterator::nextInt);

		IntSequence threeLimitTailToTwo = _123.limitTail(2);
		twice(() -> assertThat(threeLimitTailToTwo, containsInts(2, 3)));

		IntSequence threeLimitTailToThree = _123.limitTail(3);
		twice(() -> assertThat(threeLimitTailToThree, containsInts(1, 2, 3)));

		IntSequence threeLimitTailToFour = _123.limitTail(4);
		twice(() -> assertThat(threeLimitTailToFour, containsInts(1, 2, 3)));

		expecting(UnsupportedOperationException.class, () -> removeFirst(threeLimitTailToFour));
		twice(() -> assertThat(threeLimitTailToFour, containsInts(1, 2, 3)));
		twice(() -> assertThat(_123, containsInts(1, 2, 3)));

		IntSequence nineLimitTailToNone = _123456789.limitTail(0);
		twice(() -> assertThat(nineLimitTailToNone, is(emptyIterable())));

		IntSequence nineLimitTailToOne = _123456789.limitTail(1);
		twice(() -> assertThat(nineLimitTailToOne, containsInts(9)));

		IntSequence nineLimitTailToTwo = _123456789.limitTail(2);
		twice(() -> assertThat(nineLimitTailToTwo, containsInts(8, 9)));

		IntSequence nineLimitTailToThree = _123456789.limitTail(3);
		twice(() -> assertThat(nineLimitTailToThree, containsInts(7, 8, 9)));

		IntSequence nineLimitTailToFour = _123456789.limitTail(4);
		twice(() -> assertThat(nineLimitTailToFour, containsInts(6, 7, 8, 9)));
	}
	...
	public void sum() {
		...
		IntSequence fiveMaxValues = IntSequence.of(Integer.MAX_VALUE).repeat(5);
		twice(() -> assertThat(fiveMaxValues.sum(), is((long) Integer.MAX_VALUE * 5)));
	}
	...
	public void average() {
		...
		IntSequence fiveMaxValues = IntSequence.of(Integer.MAX_VALUE).repeat(5);
		twice(() -> assertThat(fiveMaxValues.average(), is(OptionalDouble.of(Integer.MAX_VALUE))));
	}

	@Test
	public void statistics() {
		twice(() -> {
			IntSummaryStatistics emptyStatistics = empty.statistics();
			assertThat(emptyStatistics.getCount(), is(0L));
			assertThat(emptyStatistics.getMin(), is(Integer.MAX_VALUE));
			assertThat(emptyStatistics.getMax(), is(Integer.MIN_VALUE));
			assertThat(emptyStatistics.getAverage(), is(0.0));
		});

		twice(() -> {
			IntSummaryStatistics fiveStatistics = _12345.statistics();
			assertThat(fiveStatistics.getCount(), is(5L));
			assertThat(fiveStatistics.getMin(), is(1));
			assertThat(fiveStatistics.getMax(), is(5));
			assertThat(fiveStatistics.getAverage(), is(3.0));
		});

		twice(() -> {
			IntSummaryStatistics oneRandomStatistics = oneRandom.statistics();
			assertThat(oneRandomStatistics.getCount(), is(1L));
			assertThat(oneRandomStatistics.getMin(), is(17));
			assertThat(oneRandomStatistics.getMax(), is(17));
			assertThat(oneRandomStatistics.getAverage(), is(17.0));
		});

		twice(() -> {
			IntSummaryStatistics twoRandomStatistics = twoRandom.statistics();
			assertThat(twoRandomStatistics.getCount(), is(2L));
			assertThat(twoRandomStatistics.getMin(), is(17));
			assertThat(twoRandomStatistics.getMax(), is(32));
			assertThat(twoRandomStatistics.getAverage(), is(24.5));
		});

		twice(() -> {
			IntSummaryStatistics nineRandomStatistics = nineRandom.statistics();
			assertThat(nineRandomStatistics.getCount(), is(9L));
			assertThat(nineRandomStatistics.getMin(), is(-7));
			assertThat(nineRandomStatistics.getMax(), is(17));
			assertThat(nineRandomStatistics.getAverage(), is(35.0 / 9));
		});

		IntSequence fiveMaxValues = IntSequence.of(Integer.MAX_VALUE).repeat(5);
		twice(() -> {
			IntSummaryStatistics fiveMaxValuesStatistics = fiveMaxValues.statistics();
			assertThat(fiveMaxValuesStatistics.getCount(), is(5L));
			assertThat(fiveMaxValuesStatistics.getMin(), is(Integer.MAX_VALUE));
			assertThat(fiveMaxValuesStatistics.getMax(), is(Integer.MAX_VALUE));
			assertThat(fiveMaxValuesStatistics.getAverage(), is((double) Integer.MAX_VALUE));
		});
	}
	...
	public void rangeOpen() {
		IntSequence rangeOpen1to6 = IntSequence.rangeOpen(1, 6);
		twice(() -> assertThat(rangeOpen1to6, containsInts(1, 2, 3, 4, 5)));

		IntSequence rangeOpen6to1 = IntSequence.rangeOpen(6, 1);
		twice(() -> assertThat(rangeOpen6to1, containsInts(6, 5, 4, 3, 2)));

		IntSequence rangeOpen_2to2 = IntSequence.rangeOpen(-2, 2);
		twice(() -> assertThat(rangeOpen_2to2, containsInts(-2, -1, 0, 1)));

		IntSequence rangeOpen2to_2 = IntSequence.rangeOpen(2, -2);
		twice(() -> assertThat(rangeOpen2to_2, containsInts(2, 1, 0, -1)));

		IntSequence maxValue = IntSequence.rangeOpen(Integer.MAX_VALUE - 3, Integer.MAX_VALUE);
		twice(() -> assertThat(maxValue,
		                       containsInts(Integer.MAX_VALUE - 3, Integer.MAX_VALUE - 2,
		                                    Integer.MAX_VALUE - 1)));

		IntSequence minValue = IntSequence.rangeOpen(Integer.MIN_VALUE + 3, Integer.MIN_VALUE);
		twice(() -> assertThat(minValue,
		                       containsInts(Integer.MIN_VALUE + 3, Integer.MIN_VALUE + 2,
		                                    Integer.MIN_VALUE + 1)));
	}

	@Test
	public void rangeOpenWithStep() {
		IntSequence rangeOpen1to6step2 = IntSequence.rangeOpen(1, 6, 2);
		twice(() -> assertThat(rangeOpen1to6step2, containsInts(1, 3)));

		IntSequence rangeOpen6to1step2 = IntSequence.rangeOpen(6, 1, 2);
		twice(() -> assertThat(rangeOpen6to1step2, containsInts(6, 4)));

		IntSequence rangeOpen_6to6step2 = IntSequence.rangeOpen(-6, 6, 3);
		twice(() -> assertThat(rangeOpen_6to6step2, containsInts(-6, -3, 0, 3)));

		IntSequence rangeOpen6to_6step2 = IntSequence.rangeOpen(6, -6, 3);
		twice(() -> assertThat(rangeOpen6to_6step2, containsInts(6, 3, 0, -3)));

		IntSequence maxValue = IntSequence.rangeOpen(Integer.MAX_VALUE - 2, Integer.MAX_VALUE, 2);
		twice(() -> assertThat(maxValue, containsInts(Integer.MAX_VALUE - 2)));

		IntSequence minValue = IntSequence.rangeOpen(Integer.MIN_VALUE + 2, Integer.MIN_VALUE, 2);
		twice(() -> assertThat(minValue, containsInts(Integer.MIN_VALUE + 2)));

		IntSequence crossingMaxValue = IntSequence.rangeOpen(Integer.MAX_VALUE - 3, Integer.MAX_VALUE, 2);
		twice(() -> assertThat(crossingMaxValue, containsInts(Integer.MAX_VALUE - 3)));

		IntSequence crossingMinValue = IntSequence.rangeOpen(Integer.MIN_VALUE + 3, Integer.MIN_VALUE, 2);
		twice(() -> assertThat(crossingMinValue, containsInts(Integer.MIN_VALUE + 3)));

		expecting(IllegalArgumentException.class, () -> IntSequence.rangeOpen(1, 6, -1));
	}
